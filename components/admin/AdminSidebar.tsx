...
const NAV = [
  { key: "dashboard", label: "Admin Dashboard", Icon: GearIcon, active: true },
  { key: "management", label: "Admin Management", Icon: ShieldUsersIcon, active: false },
];
      ...
      <nav className="flex flex-1 flex-col gap-1.5 p-4">
        {NAV.map(({ key, label, Icon, active }) => (
          <span
            key={key}
            className={cn(
              "relative flex h-10 cursor-default items-center gap-2 rounded-r-2xl px-4 text-sm transition-colors",
              active
